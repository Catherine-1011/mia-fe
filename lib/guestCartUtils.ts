...
export const getCheckoutOptions = async (): Promise<{
  ...
  try {
    ...
    const response = await fetch('http://134.199.161.119:5000/api/cart/checkout-options', {
      method: 'GET',
      headers: {
...
export const guestCartUtils = {
  ...
  calculateGuestCartTotalsFromAPI: async (
    ...
    try {
      const response = await fetch("http://134.199.161.119:5000/api/cart/calculate-guest", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export const syncGuestCartAfterLogin = async (token: string): Promise<void> => {
  ...
  try {
    await fetch('http://134.199.161.119:5000/api/cart/sync', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
